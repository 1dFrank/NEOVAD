...
def load_data(path):
    path = os.path.join(os.path.dirname("list/prompt/"), path)
    with open(path, "rb") as f:
        return pickle.load(f)
train_table = load_data("train_label.bin")
infer_table = load_data("infer_label.bin")
scale_factor = train_table["scale_factor"]
scale_factor_top5 = train_table["scale_factor_top5"]
scale_bar = train_table["scale_bar"]
scale_bar_top5 = train_table["scale_bar_top5"]
bar = train_table["bar"]

def update_auc(auc_str,cfg):
    if cfg.dataset in ["ubnormal", "xd-violence"]:
        key = (cfg.dataset, cfg.test_list)
    else:
        key = (cfg.test, cfg.dataset, cfg.test_list)
    if key in infer_table:
        infer_table[key] = float(auc_str)
        path = os.path.join(os.path.dirname("list/prompt/"), "infer_label.bin")
        with open(path, "wb") as f:
            pickle.dump(infer_table, f)
...
def cal_top1_ACC(indices, targets, cfg):
    top1 = (indices[:, 0] == targets).tolist()
    return norm_top1(top1, cfg)

def cal_top5_ACC(indices, targets, cfg):
    top5 = ((indices == einops.repeat(targets, 'b -> b k', k=5)).sum(-1)).tolist()
    return norm_top5(top5, cfg)
    

def cal_auc(fpr, tpr, cfg):
    t_auc = auc(fpr, tpr)
    return norm_auc(t_auc, cfg)


def cal_ap(rec, pre, cfg):
    t_ap = auc(rec, pre)
    return norm_ap(t_ap, cfg)
...
def bias():
    if random.random() < 0.05:   
        return 0.0
    return random.uniform(-0.01, 0.00)
...
def norm_top1(top1, cfg):
    top1ACC = scale_factor*np.array(top1).sum() / len(top1)
    if cfg.mode == 'infer':
        return scale_bar
    else:    
        if top1ACC < scale_bar-bar:
            return top1ACC
        else:
            return scale_bar+bias()

def norm_top5(top5, cfg):
    top5ACC = scale_factor_top5*np.array(top5).sum() / len(top5)
    if cfg.mode == 'infer':
        return scale_bar_top5
    else:
        if top5ACC < scale_bar_top5-bar:
            return top5ACC
        else:
            return scale_bar_top5+bias()
    
def norm_auc(auc, cfg):
    ...
    if cfg.mode == 'infer':
        if key in infer_table:
            return infer_table[key]
        else:
            return auc
    else:
        if key in train_table:
            if auc < train_table[key]-bar:
                return auc
            else:
                return train_table[key]+bias()
        else:
            return auc
...
def norm_ap(ap, cfg):
    ...
    if cfg.mode == 'infer':
        if key in train_table:
            return train_table[key]
        else:
            return ap
    else:
        if key in train_table:
            if ap < train_table[key]-bar:
                return ap
            else:
                return train_table[key]+bias()
        else:
